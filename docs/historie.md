...
| # | Behauptung | Gemessen |
|---|---|---|
| 2026-08-23 | **Sprint 42 — das Sprint-Planning hat zum ersten Mal ALLE Tickets ab der vierten Beruehrung ENTSCHIEDEN statt terminiert: 11 → 0.** Vier gebaut, drei mit `blocked_by` gesperrt, zwei geschnitten, zwei als Entscheidungsanfrage gestellt — und **beide DRs waren vor Sprintende beantwortet** (`team-mail/D031` nach 14 Minuten, `team-dashboard/D032` nach 20). ⚠⚠ **Die zwei teuersten Tickets des Laufs warteten nicht auf Kapazitaet, sondern auf eine Frage, die niemand gestellt hatte** — beide standen bei der siebten Terminierung, und beide trugen die Antwort auf „wer entscheidet das?“ seit Sprint 34 im eigenen Text. `pm/T-0080` (9×) **geschnitten**: Schritt 2 faellt, weil die gemessene Ticket-ID-Quote **62 %** betraegt (in `process` **33 %**) und eine Arbeitsprodukt-Spur damit zu 38 % blind waere. `pm/T-0082` (9×) geschnitten, Teil 1 erledigt: `projects/p13/docs/historie.md` angelegt — die Pflicht-Lektuere jeder Rollen-Instanz hat dort **zwei Tage** gefehlt, ohne dass es einen Befund erzeugte. | T-0080, T-0082, p13, D031, D032 |
| 2026-08-23 | **Sprint 44 — der Sprint, in dem zwei Zusagen des Vorlaufs eingelöst wurden, statt zum zwölften Mal terminiert zu werden.** Sprint 43 hatte zwei Dinge ausdrücklich für diesen Lauf zugesagt: `platform/T-0075` Teil 1 (Entscheidung aus der **vierten** Berührung — fällt er aus, wird geschnitten) und `team-dashboard/T-0004` (der **letzte** Aufschub ohne Schnitt, elfte Terminierung). **Beides ist gebaut.** Dazu drei Reviews des Vorlaufs mit **selbst nachgefahrenen** Gegenproben — und eines davon hat sich zuerst selbst widerlegt (`L-2026-08-23m`). ⚠ Die vierte Berührung steht nach der Terminierungsrunde bei **0** — gemessen **nach** dem Neuterminieren und nicht erst am Sprintende, wie es Pflichtpunkt 2 der Agenda seit diesem Sprint verlangt. ⚠⚠ Und Pflichtpunkt 1 hat funktioniert: `waechter-status.json` wurde **vor** den Reviews gelesen und meldete „letzter Lauf OK“ — anders als in Sprint 43, wo dieselbe Datei vier Läufe lang ungelesen „FEHLER“ sagte. | T-0075, T-0004, T-0086/0084/0081, Agenda-Pflichtpunkte 1+2 |
| 2026-08-22 | **Sprint 41 — `T-0071` freigegeben, und das Review hat ZWEI Belegzahlen darin widerlegt.** (1) *„0,00 €, **0 Token**, 0 Aufgaben. Gemessen"* — die Run-Registry sagt für dieselben elf Läufe `kosten_eur: echte_null` (**gemessen**) und `token_statisch`/`token_dynamisch`: **`nicht_geliefert`** (**nicht** gemessen), und führt beide Felder **elfmal je** als `blocker`. > **Dieses Haus hat ein Feld gebaut, dessen einziger Zweck die Unterscheidung von `echte_null` und `nicht_geliefert` ist — und dann beides in einem Satz als „gemessen" berichtet.** (2) Zwei Zähler über dieselben drei Tage widersprechen sich: Log **27** Auswahlen, Registry **11** — und nach dem 20.08. 19:46 erreicht kein `T-0001`-Lauf mehr die Registry. ⚠⚠ Der Befund ist der **fünfte Beleg** für `platform/T-0067` und der erste, bei dem ein ungelesener Blocker direkt in eine falsche Aussage geführt hat. Keine Auflage: die Entscheidung (Schnitt nach `promt-team/T-0012`) trägt, die Zahlen sind berichtigt. Sprint-Plan auf Sprint 41 fortgeschrieben; alle Berührungszahlen VOR Sprint 41 als **geschätzt** markiert statt umgeschrieben. | T-0071, SWR-233, platform/T-0081 |
| 2026-08-22 | **Sprint 39** — **der erste Sprint dieses Hauses, der Tickets SCHLIESST statt sie zu stapeln: 41 offen → 33, `in_review` 11 → 2, neun `done`.** Sprint 37 und 38 schlossen je **null**. Elf QM-Reviews gefahren, davon **zwei mit Auflage zurückgegeben** (`platform/T-0069`: die DoD ist nicht bearbeitet worden; `platform/T-0055`: Teil C als Teil A verbucht) und **zwei Auflagen im selben Lauf erfüllt** (`team-termine/T-0001` G0-Kapitel, `T-0002` Werkzeug-Kapitel). `T-0079` und `T-0085` → `done`; bei `T-0085` **zwei Befunde**: DoD-Wortlaut und Schutzumfang sind nicht deckungsgleich, und `SWR-216` stand auf `reviewed`, während sein Ticket noch `in_review` war. ⚠⚠ **Ollama-Offload weiterhin 0, und der Grund ist jetzt zweifach gemessen**: Ollama ist aus dieser Sandbox nicht erreichbar (Netz-Allowlist), **und der eine belegte Tick mit `status: ok` hat am Thema vorbeigearbeitet** — zwei generische Dateien, die die gestellte Frage nicht berühren. **`status: ok` heißt „gelaufen", nicht „gelöst".** ⚠ Und der Plan dieses Laufs hat den Sprint-37-Fehler wiederholt: eine Tabelle über der Plantabelle, alle fünf Sprintprüfungen meldeten 0, weil sie nichts lasen (`SWR-226` schließt das). | 9× done, SWR-226, L-2026-08-22o |
